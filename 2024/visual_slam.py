...
class VisualSlam:
    ...
    def calculate_epipolar_distance(self, match, fundamental_matrix):
        # Extract keypoints from the match
        kp1 = match.keypoint1
        kp2 = match.keypoint2
        # Convert keypoints to homogeneous coordinates
        pt1 = np.array([kp1[0], kp1[1], 1])
        pt2 = np.array([kp2[0], kp2[1], 1])
        # Calculate the epipolar line in the second image corresponding to the first point
        line = np.dot(fundamental_matrix, pt1)
        # Calculate the distance from the second point to the epipolar line
        distance = np.abs(np.dot(pt2, line)) / np.linalg.norm(line[:2])
        return distance

    def calculate_epipolar_distances(self, matches, fundamental_matrix):
        distances = []
        for match in matches:
            distance = self.calculate_epipolar_distance(match, fundamental_matrix)
            distances.append(distance)
        return distances

    def summarize_epipolar_distances(self):
        ip = self.current_image_pair
        F=ip.determine_fundamental_matrix(ip.filtered_matches)
        distances=self.calculate_epipolar_distances(ip.filtered_matches,F)
        mean_distance = np.mean(distances)
        std_distance = np.std(distances)
        print("Epipolar Summary Statistics:")
        print(f"Mean Distance: {mean_distance}")
        print(f"Standard Deviation: {std_distance}")

    def plot_camera_trajectory(self):
        import matplotlib.pyplot as plt
        # Extract poses from cameras and calculate relative translations
        relative_translations = []
        last_translation = None
        for camera in self.map.cameras:
            if camera.frame_id in self.camera_dict:
                pose = self.camera_dict[camera.frame_id].pose()
                translation = pose[:3, 3]
                if last_translation is not None:
                    relative_translation = translation #- last_translation
                    relative_translations.append(relative_translation[:2])  # Extract only X and Y components
                last_translation = translation
            else:
                print(f"Camera with frame_id {camera.frame_id} not found in camera_dict.")
        
        if relative_translations:
            relative_translations = np.array(relative_translations)

            # Accumulate relative translations to get final trajectory
            cumulative_trajectory = np.cumsum(relative_translations, axis=0)

            # Plot trajectory
            fig = plt.figure()
            ax = fig.add_subplot(111)
            ax.plot(cumulative_trajectory[:, 0], cumulative_trajectory[:, 1], marker='o', linestyle='-')
            ...
            print("No camera poses found for plotting.")
    ...
    def run(self):
        ...
        for idx, filename in enumerate(list_of_files):
            ...
            #self.show_map_points("show map points:")
            ...
            if k == ord('b'):
                ...
                self.freeze_nonlast_cameras()
                self.print_camera_details()
                self.summarize_epipolar_distances()
